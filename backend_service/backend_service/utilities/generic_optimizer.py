...
def genetic_algorithm(
    ...
    Args:
        ...
        bounds ([list, [min, max]]): a listof lists for lower and upper limits
        break_accuracy (float): Min Difference to break,
        Defaults to 0.005.
        model ([type]): a mlflow prediction model
    ...
    best = 0
    best_eval = objective(target=target, X=decode(bounds, n_bits, pop[0]), model=model)
    ...
    for gen in range(n_iter):
        if best_eval <= break_accuracy:
            ...
            scores = [objective(target=target, X=d, model=model) for d in decoded]
